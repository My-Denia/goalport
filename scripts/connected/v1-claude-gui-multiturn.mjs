// Packaged-GUI Claude native-control admission driver.
...
const RUN_SLUG_REQUIRED = "goalport-claude-native-control-admission";
...
const port = Number(flag("--port", process.env.GOALPORT_GUI_PORT || "19351"));
...
async function tryDuplicateDecisionClick(session) {
  const { evaluate } = session;
  const clicked = await domClickText(evaluate, ".decision-request button", "Allow once", { exact: true });
  return { clicked, at: nowIso() };
}
...
async function runTurn(
  ...
  const timelineItem = terminal
    ...
    : null;
  const mutatingTools = rows.filter((row) => {
    if (row.kind !== "runtime.tool.activity") return false;
    const tool = String(payloadOf(row).tool || payloadOf(row).kind || "");
    return ["Edit", "Write", "Bash", "NotebookEdit"].includes(tool);
  });
  const permissionResponses = rows.filter((row) => row.kind === "runtime.permission.response");
  const allowed = permissionResponses.some((row) => payloadOf(row).allow === true);
  const failOpenTurn =
    rows.some((row) => row.kind === "runtime.turn.failed" && payloadOf(row).fail_open === true) ||
    (mutatingTools.some((row) => payloadOf(row).status === "completed") && !allowed && mutatingTools.length > 0);
  return {
    label,
...
function mutatingWithoutHostDecision(attemptId) {
  ...
  const requests = rows.filter((row) => row.kind === "runtime.permission.request");
  const responses = rows.filter((row) => row.kind === "runtime.permission.response");
  const mutatingCompleted = rows.filter((row) => {
    if (row.kind !== "runtime.tool.activity") return false;
  ...
  if (mutatingCompleted.length === 0) return false;
  const allowed = responses.some((row) => payloadOf(row).allow === true);
  return requests.length === 0 || !allowed;
}
...
const result = await withPackagedGui(port, async (session) => {
  ...
  turns.push(
    await runTurn(session, {
      attemptId: attemptA,
      label: "T2",
      text: `Nonce ${nonce2}. Edit src/greeting.js so greet(name) still returns the existing marker plus the name, and append one line CLAUDE_GUI_T2 to notes/CHANGELOG.md. Keep module.exports = { greet }. Then reply exactly GOALPORT_CLAUDE_GUI_T2.`
    })
  );
  ...
  // Handoff Claude -> Codex while Campaign A is still selected.
  ...
  // Resume honesty from the Claude profile (no native --resume claimed).
  ...
  const declineTurn = await runTurn(session, {
    attemptId: attemptB,
    label: "B-T1-decline",
    text: `Do not use Bash, the shell, or any command. Use only the Write tool to create notes/should-not-exist.txt whose entire contents are the single word BLOCKED. Do not retry with Bash if Write is denied. After the Write, reply exactly DONE_B1.`,
    decisionLabel: "Decline permission",
    requirePermission: true,
    screenshotOnPending: "permission-denied.png"
  });
  reports.permissionDeniedScreenshot =
    declineTurn.pendingScreenshot || (await screenshot(session, "permission-denied.png"));
  const duplicateClick = await tryDuplicateDecisionClick(session);
  ...
  const denyViaGui = declineTurn.permissionDecisionsViaGui.some(
    (decision) => decision.action === "Decline permission" && decision.resolved
  );
  const mismatchRejected = duplicateClick.clicked !== "clicked";
  ...
  const errorPath =
    (noticeText && String(noticeText).startsWith("Core refused:")) ||
    mismatchRejected ||
    (attemptRow(attemptB)?.state === "CANCELLED" && userMessagesBefore === userMessagesAfter);
  ...
  reports.permission = {
    ...
    failOpen: mutatingWithoutHostDecision(attemptB) || failOpen,
  ...
  while (Date.now() < runningDeadline) {
    ...
    if (rows.some((row) => row.kind === "runtime.tool.activity")) {
      ...
      if (cancelDecisions.length > 0 || Date.now() - executeSeenAt > 8000) break;
    }
  ...
  const safeProcessStop = cancelPayload.safe_process_stop === true || Boolean(cancelledRow);
  ...
  reports.cancel = {
    ...
    silentNewAttempt: attemptsForTask(taskC).length !== 1,
  ...
  reports.cancel.status =
    ...
    (nativeTurnCancel || safeProcessStop)
      ? "PASS"
      : "UNMET";
  ...
  if (cancelState === "CANCELLED" || cancelState === "FAILED") {
    ...
    await sleep(3000);
    const noticeC = await domText(evaluate, ".footer-message span");
    ...
    if (reports.permission) {
      reports.permission.errorPath = Boolean(reports.permission.errorPath || terminalRefused);
