...
**Core** (`crates/goalport-core`). The only writer of GoalPort state. It serves a local Windows Named Pipe, using the default Windows pipe security, whose name is derived from the profile and the Core binary hash. It does not open a TCP listener. Frames are length-prefixed JSON capped at 16 MiB. Every command reply carries a full snapshot. A background thread keeps persisting Runtime events whether or not a window is attached.
