// Packaged-GUI Claude deny/fail-open admission driver (this run).
...
// Do not edit scripts/connected/v1-claude-gui-multiturn.mjs slug lock.
...
const RUN_SLUG_REQUIRED = "goalport-claude-deny-fail-open-admission";
...
async function clickDeclineThenDuplicateSameId(session, decisionId) {
  ...
  const clicked = await evaluate(
    `(() => {
      ...
      const row = rows[0];
      if (!row) return { first: "missing", second: "missing", rowCount: 0, wantId };
      const decline = [...row.querySelectorAll("button")].find(
        (node) => (node.textContent || "").trim() === "Decline permission"
      );
      if (!decline || decline.disabled) {
        return { first: decline && decline.disabled ? "disabled" : "missing", second: "missing", rowCount: rows.length, wantId };
      }
      decline.click();
      const second = [...row.querySelectorAll("button")].find(
        (node) => (node.textContent || "").trim() === "Decline permission"
      );
      if (!second) return { first: "clicked", second: "missing", rowCount: rows.length, wantId };
      second.click();
      return {
        first: "clicked",
        second: "clicked",
...
async function captureRefusalNotices(session, timeoutMs = 8000) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const snap = await snapshot(session.evaluate);
    const notices = Array.isArray(snap.notices) ? snap.notices : [];
    const hit = notices.find((notice) => {
      const text = String(notice || "");
      return (
        text.startsWith("Core refused:") &&
        (/only pending/i.test(text) || /duplicate/i.test(text) || /InvalidRequest/i.test(text) || /Denied/i.test(text))
      );
    });
    if (hit) {
      return {
        kind: "stale-or-duplicate-rejected",
        notice: hit,
  ...
  return {
    kind: "unobserved",
    notice: null,
    notices: Array.isArray(snap.notices) ? snap.notices.slice(0, 8) : [],
    at: nowIso()
  };
}
...
const result = await withPackagedGui(port, async (session) => {
  ...
  // Campaign B: deny + error-path. Two GUI clicks on the same captured pending id.
  ...
  if (!pendingDecision) {
    reports.permission = {
      ...
      operationId,
      host: "electron-packaged",
      driver: "scripts/connected/v1-claude-gui-deny-admission.mjs",
      status: "UNMET",
  ...
  const errorPathCapture = await captureRefusalNotices(session, 8000);
  const secondClicked = duplicateClick.clicked === "clicked" && duplicateClick.clicked !== "missing";
  const noticeTextAtClick = String(errorPathCapture.notice || "");
  const noticeMatches =
    /only pending/i.test(noticeTextAtClick) ||
    /duplicate/i.test(noticeTextAtClick) ||
    /InvalidRequest/i.test(noticeTextAtClick);
  const mismatchRejected = secondClicked && noticeMatches === true;
  const errorPath = {
    kind: mismatchRejected ? "stale-or-duplicate-rejected" : errorPathCapture.kind,
    notice: errorPathCapture.notice,
    notices: errorPathCapture.notices,
    decisionId: capturedDecisionId,
    capturedAtUtc: errorPathCapture.at
  };
