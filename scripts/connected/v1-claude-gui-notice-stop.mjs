// Packaged-GUI Claude notice/stop/dup admission driver (this run).
// Drives the packaged React DOM only. No Core message injection.
// Do not edit v1-claude-gui-live-deny.mjs / v1-claude-gui-deny-admission.mjs /
// v1-claude-gui-multiturn.mjs slug locks.
...
const DRIVER_REL = "scripts/connected/v1-claude-gui-notice-stop.mjs";
const RUN_SLUG_REQUIRED = "goalport-claude-notice-stop-dup-admission";
...
const PACKAGED_EXE = resolve(EVID, "electron-package/GoalPort-win32-x64/GoalPort.exe");
const PACKAGED_CORE = resolve(
  EVID,
  "electron-package/GoalPort-win32-x64/resources/goalport-core.exe"
);
const CLAUDE_HOME = resolve(homedir(), ".claude");
const FORBIDDEN_CORE = "4c435cfe59f0068221e8963070f7a98ce8ba1033d3ea0d78c8868ab857c7dd82";
...
if (packagedCoreSha256 === FORBIDDEN_CORE) {
  throw new Error("this freeze still hashes to 4c435cfe; edits did not land");
}
...
if (!reopenOnly) try {
  await withPackagedGui(port, async (session) => {
    ...
    await createCampaign(evaluate, `Claude notice-stop admission A ${operationId}`);
    ...
    reports.permission = {
      ...
      usageEvidence: {
        class: "gui",
        boundaryStates: ["permission-denied", "error-path"],
        permissionDenied: true,
        errorPath: Boolean(boundB)
      },
      allowOnce: {
    ...
    const xorCount = [nativeTurnCancel, safeProcessStop, unsupportedStop].filter(Boolean).length;
    const ac6cClosed = {
    ...
    reports.handoffCoreReport = {
      ...
      oldSessionHash,
      newSessionHash,
      coreArtifact: `${EVID_REL}/electron-package/GoalPort-win32-x64/resources/goalport-core.exe`,
    ...
    reports.multiruntime = {
      ...
      status:
        ...
        packagedCoreSha256 &&
        packagedCoreSha256 !== FORBIDDEN_CORE
          ? "PASS"
          : "UNMET"
    };
...
try {
  ...
  await withPackagedGui(port, async (session) => {
    ...
    if (composerReady) {
      ...
      const followDeadline = Date.now() + Math.min(turnTimeoutMs, 180000);
      while (Date.now() < followDeadline) {
        ...
        const rows = attemptId ? eventsAfter(attemptId, 0) : [];
        if (rows.some((row) => TERMINAL_KINDS.includes(row.kind) && row.seq > (snap.cursor || 0))) {
          break;
        }
        await sleep(500);
      }
    ...
    reopenSample.followUpNativeTurn = attemptId
      ? eventsAfter(attemptId, snap.cursor || 0).some((row) => row.kind === "runtime.turn.started")
      : false;
  });
} catch (error) {
