# Historical Desktop admission notes

This is the repository README preserved before the independent Electron RC delivery.
All "this run", "current", versions, hashes, admission rulings and launch paths below
refer to their original historical sessions. They are not build prerequisites,
current package identities, or admission claims for the repository-built RC.
Use the root README for current build, launch, data and validation instructions.
Historical run folders and databases are neither imported nor modified by the RC.
